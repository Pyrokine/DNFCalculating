...
class skill6(归元_气功师_男主动技能):
    ...
    学习间隔 = 2
    等级精通 = 50
    基础等级 = min(int((归元_气功师_男等级 - 所在等级) / 学习间隔 + 1), 等级精通)
    # 基础等级 = 38
    # 基础 = 652
    # 成长 = 74
...
class skill15(归元_气功师_男主动技能):
    ...
    攻击次数1 = 17
    数据2 = [0, 229, 252, 275, 299, 322, 345, 369, 392, 415, 438, 462, 485, 508, 531, 555, 578, 601, 624, 648, 671, 694, 718, 741, 764, 787, 811, 834, 857, 880, 904, 927, 950, 974, 997, 1020, 1043,
           1067, 1090, 1113, 1136, 1160, 1183, 1206, 1229, 1252, 1275, 1298, 1321, 1344, 1367, 1390, 1413, 1436, 1459, 1482, 1505, 1528, 1551, 1574, 1597]
    攻击次数2 = 1.5
    风雷啸 = 1.6
    CD = 21
    TP成长 = 0.10
...
for i in skill_list:
    if i.是否有伤害 == 1 and i.是否有护石 == 1:
        归元_气功师_男护石选项.append(i.名称)

归元_气功师_男符文选项 = ['无']
for i in skill_list:
    if i.所在等级 >= 20 and i.所在等级 <= 80 and i.所在等级 != 50 and i.是否有伤害 == 1:
        归元_气功师_男符文选项.append(i.名称)
        # 归元_气功师_男符文选项.append('幻影爆碎') #符文计算报错，被动技能没有倍率


class character(py.lite.char_base):
    def 角色賦予(self):
        self.attr["实际名称"] = '归元_气功师_男'
        self.attr["角色"] = '格斗家(男)'
        self.attr["职业"] = '气功师'
        ...
        self.attr["主BUFF"] = 1.43
        self.attr["BUFF属强"] = 86

        self.attr["远古记忆"] = 10

    def 角色数据输入(self):
        self.attr["技能SP等级"] = [1, 11, 20, 11, 4, 100, 38, 46, 46, 10, 25, 1, 10, 25, 36, 33, 31, 12, 17, 18, 16, 13, 5, 6, 2]
        self.attr["技能TP等级"] = [0, 0, 0, 0, 0, 0, 5, 5, 5, 1, 0, 0, 0, 0, 5, 5, 5, 0, 3, 5, 0, 0, 0, 0, 0]
        self.attr["技能释放次数"] = ['', '', '', '', '', '', '3', '3', '3', '3', '0', '4', '10', '1', '0', '2', '1', '1', '1', '1', '1', '1', '1', '1', '1']
        self.attr["技能宠物次数"] = [0, 0, 0, 0, 0, 0, 1, 1, 1, 2, 0, 2, 5, 0, 0, 0, 1, 0, 1, 1, 0, 1, 1, 0, 1]
        ...
        self.attr["左槽白金技能"] = "念兽：龙虎啸"
        self.attr["右槽白金技能"] = "念气环绕"
        self.attr["时装上衣技能"] = "风雷啸"
    ...
    def 面板魔法攻击力(self):
        面板魔法攻击 = self.attr["主BUFF"] * (self.attr["魔法攻击力"] + self.attr["进图魔法攻击力"]) * (1 + self.attr["百分比三攻"]) * 1.22
        武器类型 = self.attr["武器类型"]

        for i in self.attr["技能栏"]:
            try:
                面板魔法攻击 *= i.魔法攻击力倍率(武器类型)
            except:
                pass
        return 面板魔法攻击

    def 被动倍率计算(self):
        self.attr["技能栏"][skill_sn['分身']].基础个数 = self.attr["技能栏"][skill_sn['幻影爆碎']].TP等级
        super().被动倍率计算()
